...
Various occupancy detection approaches have been explored in the literature, including

Bluetooth-based methods \cite{park2018}, WiFi and BLE packet capture \cite{longo2019},

sensor fusion techniques \cite{hobson2019}, and comparative evaluations of different counting

methods \cite{yang2018}. Comprehensive frameworks for occupancy indicators have also been

proposed to optimize building operation \cite{mo2019}, and IoT sensor setups have been

developed to track occupancy patterns in ofﬁce spaces \cite{li2019}. Chaudhari et al. conducted a comprehensive survey of occupancy detection technologies
...
erate only when and where occupants are actually present \cite{chaudhari2024}. Song et al. further examined
...
and monitoring technologies are essential for closing the gap between designed and actual building performance \cite{song2024}. Waghchoure et al. demonstrated a practical classroom automa-

tion system that uses PIR sensing and camera activation to trigger appliance control, con-

ﬁrming that low-cost IoT components can be deployed in educational settings to reduce

unnecessary electricity use \cite{waghchoure2025}.
...
use of the ESP32 for classroom and laboratory automation is well-established in the

literature \cite{ingole2024,anderson2024}, and low-cost environmental sensing with

temperature and light sensors has been demonstrated in multiple educational IoT

deployments \cite{basheer2021,simanungkalit2020}. The

artiﬁcial-intelligence layer uses camera frames and a YOLO-based model to estimate the

number of occupants in the laboratory. Recent advances in lightweight vision architectures

have demonstrated improved indoor detection performance through modiﬁed YOLO variants

and attention-guided feature pyramids \cite{choi2023,zhang2024}, while edge-optimized

convolutional architectures have enabled real-time person counting on resource-constrained

systems \cite{mitchell2024,lee2024}. The backend layer receives sensor and occupancy
...
not conﬁrm whether occupants are present, whether natural light is sufﬁcient, whether ven-
...
\cite{chaudhari2024,waghchoure2025}. GreenLab follows this direction by using embedded sensor readings for immediate
...
ment, laboratory scheduling, and approval workﬂows for assignment changes so that room
...
of multimodal sensing and governed control paths \cite{challa2025}. Chaudhari et al. conﬁrmed that

occupancy-aware automation reduces building energy waste, but noted that sensor place-

ment, real-time constraints, and privacy considerations remain practical challenges \cite{chaudhari2024}.

The absence of a uniﬁed system means that each of these concerns is currently ad-
...
ﬁne when a laboratory should be active but cannot conﬁrm that the assigned instructor is
...
enabling each tier to evolve independently while maintaining well-deﬁned interface con-
...
\begin{itemize}  \item Objective 1: Deﬁne a layered four-tier architecture with clear interface con-
\end{itemize}tracts. Establish a layered architecture that separates hardware sensing and actuation,

AI processing, backend decision-making and persistence, and frontend presentation

into four distinct tiers. This architectural separation aligns with established patterns

for IoT-based building automation systems \cite{kodali2017,rao2021} and facilitates

independent maintainability and scalability \cite{davies2024,fischer2025}. Each tier communicates through well-deﬁned interfaces so
...
\begin{itemize}  \item Objective 3: Design occupancy detection using multimodal input. Design oc-
\end{itemize}cupancy detection using multimodal input that combines ESP32 sensor readings,
...
awareness, with YOLO-based visual counting from an IP camera stream. This multimodal

approach is supported by research demonstrating that sensor fusion improves occupancy

estimation reliability \cite{tanaka2024,patel2024,lindqvist2025}, and that combining

environmental with visual data reduces false positives compared to single-modality

systems \cite{nakajima2025,dupont2025}. Multimodal
...
permissions, laboratory assignment change handling with approval workﬂows, fault detec-
...
sources and are not presented as GreenLab results \cite{chaudhari2024,zhang2026}. The scope draws on research covering privacy-preserving computer vision \cite{rossi2024}, deep learning for occupancy estimation \cite{kim2023,wang2020}, and predictive thermal comfort models \cite{zhao2024}. The scope also excludes physical
...
vance and a comparative analysis that identiﬁes the literature gap addressed by GreenLab.
...
speciﬁcations, database design with eleven tables, REST API design with eighteen end-
