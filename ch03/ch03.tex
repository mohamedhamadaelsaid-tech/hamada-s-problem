...
that no single reviewed system closes: the combination of multimodal embedded sensing, YOLO-

based visual occupancy counting, a rule-based backend automation engine, a role-based
...
every functional and non-functional requirement is cited by its fixed identifier (for example FR-

LC-03 or NFR-SEC-01) so that it can be traced unambiguously into the design, implementation,
...
distinct responsibility and exposing a well-defined interface to its neighbours. This architectural

approach is consistent with established IoT energy management frameworks for institutional

environments \cite{wibowo2023,prasad2024}, where sensor-to-cloud pipelines with

distributed edge processing have been shown to improve both reliability and maintainability

\cite{mwangi2024,patel2024b}. The use of low-power microcontrollers for localized edge

processing aligns with implementations that prioritise energy eﬃciency at the device level

whilst maintaining centralised governance through cloud or server-side components
\cite{anderson2024}., as summarised in

Table 3.1.

\begin{longtable}{|L{1.4cm}|L{3.4cm}|L{3.4cm}|L{5.2cm}|}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/greenlab-four-tier-architecture.png}
	\caption{GreenLab four-tier system architecture and data flow}
	\label{fig:3-1}
\end{figure}
...
through three connected sensors --- a DHT11 temperature and humidity sensor, an LDR light-

dependent resistor, and an HC-SR04 ultrasonic distance sensor --- and acts on the physical
...
Studies on scalable sensor network architectures for institutional deployments \cite{foster2024}

and cloud-managed multi-sensor hubs \cite{patel2024b} conﬁrm that this decoupled approach

is essential for maintaining operational continuity as the number of monitored spaces grows,

while edge-computed ambient intelligence frameworks \cite{gomez2025} provide additional

validation for distributing inference across tiers rather than centralising all processing.
...
\begin{longtable}{|L{3.8cm}|L{2.8cm}|L{6.0cm}|}
...
Relays 2--4 at their last commanded state rather than guessing (FR-FH-02). Grace periods --- the 3-

second HC-SR04 debounce (FR-EE-02) and the 15/30-second sensor and camera timeout
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/business-model-canvas.jpeg}
	\caption{GreenLab Business Model Canvas}
	\label{fig:3-2}
\end{figure}
...
routes, while direct sales and third-party integration represent the longer-term path once the four-

tier architecture has been validated at the scale Section 3.7.6 targets.
...
\begin{longtable}{|L{3.2cm}|L{8.5cm}|}
...
\begin{longtable}{|L{2.3cm}|L{2.8cm}|L{3.6cm}|L{3.7cm}|}
...
3.2.2 explained, GreenLab's governance model hinges on knowing exactly which actions are

attributable to a person.


% --- FIGURE PLACEHOLDER: Figure 3.3 ---
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/use-case-diagram.png}
	\caption{GreenLab use case diagram showing actors and system interactions}
	\label{fig:3-3}
\end{figure}
...
\end{itemize}
3.6.1 onward) that keeps SensorReadings current and is the precondition for every other
...
\begin{longtable}{|L{1.5cm}|L{3.2cm}|L{2.8cm}|L{4.5cm}|L{2.5cm}|}
	\caption{Use case descriptions and traceability to functional requirements --- UC-01 to UC-06.} \label{tab:3-5}                                                                                   \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{ID} & \textbf{Use Case}                       & \textbf{Actor(s)}         & \textbf{Description}                                                 & \textbf{Traced Requirements}           \\
	\hline
	\endfirsthead
	\hline
	\rowcolor{tableheadergray}
	...
	\hline
	\endlastfoot
	UC-01       & Login to Dashboard                      & Instructor, Administrator & Authenticate and receive a role-scoped JWT.                          & FR-AC-01, FR-AC-02                     \\
	\hline
	UC-02       & View Real-Time Monitoring Data          & Instructor, Administrator & Display live sensor, occupancy, and relay state for one or all labs. & FR-MD-01--FR-MD-04                     \\
	\hline
	UC-03       & Manual Override Control Lights and Fans & Instructor, Administrator & Issue a timed override on a specific relay, superseding automation.  & FR-MO-01--FR-MO-04, FR-LC-02, FR-VC-04 \\
	\hline
	UC-04       & Submit Laboratory Transfer Request      & Instructor                & Request a session move to a different laboratory.                    & TransferRequests table; Endpoint 13    \\
	\hline
	UC-05       & Manage Laboratory Schedule              & Administrator             & Create or update a laboratory's session schedule.                    & Endpoint 12                            \\
	\hline
	UC-06       & Approve/Reject Transfer Requests        & Administrator             & Review and resolve a pending transfer request.                       & Endpoints 14--16                       \\
	\hline
\end{longtable}

\begin{longtable}{|L{1.5cm}|L{3.2cm}|L{2.8cm}|L{4.5cm}|L{2.5cm}|}
	\caption{Use case descriptions and traceability to functional requirements --- UC-07 to UC-13.} \label{tab:3-6}                                                                                                \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{ID} & \textbf{Use Case}                               & \textbf{Actor(s)}           & \textbf{Description}                                                    & \textbf{Traced Requirements}           \\
	\hline
	...
	\hline
	\endlastfoot
	UC-07       & Manage User Accounts                            & Administrator               & Create or update user accounts and role assignment.                     & Endpoints 17--19, FR-AC-04             \\
	\hline
	UC-08       & View System Logs and Reports                    & Administrator               & Inspect the audit trail and active fault events.                        & FR-FH-03, FR-FH-04, AuditLog           \\
	\hline
	UC-09       & Automatic Occupancy-Based Lighting Control      & Hardware System (Automatic) & Apply the lighting decision matrix on every sensor cycle.               & FR-LC-01, FR-LC-03, FR-LC-04           \\
	\hline
	UC-10       & Automatic Temperature-Based Ventilation Control & Hardware System (Automatic) & Apply the fan activation table on every sensor cycle.                   & FR-VC-01--FR-VC-04                     \\
	\hline
	UC-11       & Continuous Monitoring                           & Hardware System (Automatic) & Poll all three sensors every 5 seconds.                                 & FR-SI-01--FR-SI-04, FR-EE-01--FR-EE-04 \\
	\hline
	UC-12       & End of Session Reset                            & Hardware System (Automatic) & Return relays to OFF once occupancy is Empty and no override is active. & FR-LC-03, FR-VC-03                     \\
	\hline
	UC-13       & Perform Occupancy Detection (YOLO-based)        & AI Processing System        & Run YOLOv8n inference and report a classified person count.             & FR-OD-01--FR-OD-04                     \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: System Initialisation and Startup.} \label{tab:3-7}                                                                                                                       \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                                & \textbf{Priority} \\
	\hline
	\endfirsthead
	\hline
	\rowcolor{tableheadergray}
	...
	\hline
	\endlastfoot
	FR-SI-01       & The ESP32 shall verify all three sensor interfaces on power-up before entering the normal polling loop; if any sensor fails, log fault and proceed. & P1                \\
	\hline
	FR-SI-02       & The ESP32 shall attempt Wi-Fi connection with up to 3 retries on startup; if all fail, enter Degraded Mode with all relays holding last state.      & P1                \\
	\hline
	FR-SI-03       & On first successful Wi-Fi connection after startup, the ESP32 shall retrieve the current active relay command from GET /relay/state/\{labId\}.      & P1                \\
	\hline
	FR-SI-04       & The backend shall reject any sensor POST from an ESP32 presenting an invalid or expired Service-role JWT, returning HTTP 401.                       & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Entry and Exit Detection.} \label{tab:3-8}                                                                                                                \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-EE-01       & The ESP32 shall classify an entry event when an HC-SR04 reading drops below 200 cm and increment the local occupancy counter.       & P1                \\
	\hline
	FR-EE-02       & The ESP32 shall enforce a minimum 3-second debounce interval between consecutive HC-SR04 entry or exit events.                      & P1                \\
	\hline
	FR-EE-03       & The ESP32 shall include the current occupancy counter value in every sensor POST payload sent to /sensor/readings.                  & P2                \\
	\hline
	FR-EE-04       & If the HC-SR04 returns no echo within 30 ms (out of effective range), the ESP32 shall record the reading as null in SensorReadings. & P2                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Occupancy Detection and Classification.} \label{tab:3-9}                                                                                                                        \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                                      & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-OD-01       & The Python AI service shall run YOLOv8n inference every 5 seconds per lab, filter detections to class 0 with confidence >= 0.6.                           & P1                \\
	\hline
	FR-OD-02       & The ASP.NET Core backend shall map the received person count to exactly one of four classifications: Empty (0), Low (1--5), Moderate (6--15), High (>15). & P1                \\
	\hline
	FR-OD-03       & If the IP camera stream is unreachable, the AI service shall POST a CAMERA\_UNAVAILABLE fault and cease sending occupancy updates.                        & P1                \\
	\hline
	FR-OD-04       & If YOLO and HC-SR04 classifications disagree by more than one tier, the backend shall use the higher classification as the active value.                  & P2                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Lighting Control.} \label{tab:3-10}                                                                                                                                     \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                              & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-LC-01       & The automation engine shall evaluate the lighting decision matrix on every sensor cycle and issue a relay command only when state changes.        & P1                \\
	\hline
	FR-LC-02       & When an active OverrideCommand exists for Relay 1 or Relay 2, the automation engine shall not issue any lighting relay commands.                  & P1                \\
	\hline
	FR-LC-03       & When occupancy classification is Empty and no active override exists for light relays, the automation engine shall command both light relays OFF. & P1                \\
	\hline
	FR-LC-04       & Every relay state change shall produce one row in RelayStates (CommandSource = Automation or Override) and one row in AuditLog.                   & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Ventilation Control.} \label{tab:3-11}                                                                                                                     \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                 & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-VC-01       & The automation engine shall evaluate the fan activation table on every sensor cycle and command Relay 3 and Relay 4 accordingly.     & P1                \\
	\hline
	FR-VC-02       & When temperature zone is Hot (above 30 degreesC), the automation engine shall command Relay 3 (Fan 1) to ON regardless of occupancy. & P1                \\
	\hline
	FR-VC-03       & When occupancy is Empty and temperature zone is Cool or Comfortable, both Relay 3 and Relay 4 shall be commanded OFF.                & P1                \\
	\hline
	FR-VC-04       & When an active OverrideCommand exists for Relay 3 or Relay 4, the automation engine shall not issue any ventilation relay commands.  & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Monitoring and Dashboard.} \label{tab:3-12}                                                                                                                                             \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                                              & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-MD-01       & The Angular dashboard shall refresh each lab state every 10 seconds via GET /labs/\{id\}, showing LDR value/zone, temperature, humidity, occupancy, relay states. & P1                \\
	\hline
	FR-MD-02       & An instructor shall only view data for the LabId in their JWT AssignedLabId claim; a mismatched GET /labs/\{id\} request returns HTTP 403.                        & P1                \\
	\hline
	FR-MD-03       & An administrator shall view LabStatusCards for all active laboratories from one view, with fault banners visible without scrolling.                               & P2                \\
	\hline
	FR-MD-04       & Active fault events shall appear as a styled banner showing fault type, timestamp, and a Resolve button visible only to administrators.                           & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Manual Override.} \label{tab:3-13}                                                                                                                                        \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                                & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-MO-01       & An instructor shall issue a manual override for any relay in their assigned lab only, specifying relay target, desired state, duration, and reason. & P1                \\
	\hline
	FR-MO-02       & An administrator shall issue a manual override for any relay in any lab; an administrator override supersedes any existing instructor override.     & P1                \\
	\hline
	FR-MO-03       & When an override ExpiresAt is reached, the backend shall set IsActive = 0 and resume automation engine control of the affected relay.               & P1                \\
	\hline
	FR-MO-04       & Every override creation, automatic expiry, and manual cancellation shall be recorded in AuditLog with ActorId, ActorRole, and Outcome.              & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Fault Handling and Safety.} \label{tab:3-14}                                                                                                                                               \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                                                 & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-FH-01       & The backend shall generate SENSOR\_TIMEOUT if no SensorReadings row arrives within 15 seconds, and CAMERA\_TIMEOUT if no occupancy update arrives within 30 seconds. & P1                \\
	\hline
	FR-FH-02       & When any fault is active, the automation engine shall hold safe state: Relay 1 ON (lights), Relays 2--4 hold last known state.                                       & P1                \\
	\hline
	FR-FH-03       & Active faults appear as fault banners on the relevant LabStatusCard; administrators see all labs; instructors see their assigned lab only.                           & P1                \\
	\hline
	FR-FH-04       & An administrator resolves a fault via PUT /faults/\{id\}/resolve; the backend sets IsResolved = 1, ResolvedAt = UTC now, ResolvedByAdminId.                          & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.0cm}|L{12.0cm}|L{1.5cm}|}
	\caption{Area: Authentication and Access Control.} \label{tab:3-15}                                                                                                                                     \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{FR-ID} & \textbf{Requirement}                                                                                                                                               & \textbf{Priority} \\
	\hline
	...
	\hline
	\endlastfoot
	FR-AC-01       & POST /auth/login shall validate username and verify password against bcrypt PasswordHash; on success return a JWT signed with HMAC-SHA256, expiring in 60 minutes. & P1                \\
	\hline
	FR-AC-02       & Every endpoint except POST /auth/login requires a valid non-expired JWT bearer token; missing/invalid token returns HTTP 401.                                      & P1                \\
	\hline
	FR-AC-03       & The backend shall enforce that an instructor's JWT AssignedLabId claim matches the labId in every request; any mismatch returns HTTP 403.                          & P1                \\
	\hline
	FR-AC-04       & All passwords shall be stored exclusively as bcrypt hashes; plaintext passwords shall never be stored, logged, or returned.                                        & P1                \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.5cm}|L{5.0cm}|L{3.2cm}|L{4.0cm}|}
	\caption{Non-functional requirements --- Performance.} \label{tab:3-16}                                                                                                                   \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{NFR-ID} & \textbf{Requirement}                                                                         & \textbf{Design Target} & \textbf{Rationale}                              \\
	\hline
	\endfirsthead
	\hline
	\rowcolor{tableheadergray}
	...
	\hline
	\endlastfoot
	NFR-PERF-01     & Backend responds to all API requests under normal load (<= 20 labs, <= 50 concurrent users). & < 500 ms               & Dashboard 10 s polling must feel instantaneous. \\
	\hline
	NFR-PERF-02     & End-to-end sensor-reading -> relay-command latency under normal load.                        & <= 2 seconds           & One 5 s polling cycle must complete within it.  \\
	\hline
	NFR-PERF-03     & Python AI service completes one full YOLOv8n inference cycle per lab.                        & <= 5 seconds           & Keeps occupancy fresh every polling cycle.      \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.5cm}|L{5.0cm}|L{3.2cm}|L{4.0cm}|}
	\caption{Non-functional requirements --- Reliability.} \label{tab:3-17}                                                                                                                                              \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{NFR-ID} & \textbf{Requirement}                                                                              & \textbf{Design Target}                 & \textbf{Rationale}                                    \\
	\hline
	...
	\hline
	\endlastfoot
	NFR-REL-01      & ESP32 in Degraded Mode maintains last known relay state without reset until connectivity returns. & No time limit                          & Prevents relay flapping on intermittent network.      \\
	\hline
	NFR-REL-02      & Backend uptime during lab operating hours (08:00--22:00, Mon--Sat).                               & >= 99\%                                & Faculty operations depend on continuous availability. \\
	\hline
	NFR-REL-03      & On ESP32 reconnection, device retrieves current relay command before resuming automation.         & Within 1 poll cycle after reconnection & Prevents stale relay states.                          \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.5cm}|L{5.0cm}|L{3.2cm}|L{4.0cm}|}
	\caption{Non-functional requirements --- Security.} \label{tab:3-18}                                                                                                                            \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{NFR-ID} & \textbf{Requirement}                                                                        & \textbf{Design Target} & \textbf{Rationale}                                     \\
	\hline
	...
	\hline
	\endlastfoot
	NFR-SEC-01      & All tier communication uses HTTPS; backend redirects any HTTP request to HTTPS.             & TLS 1.2+               & Protects JWT tokens and sensor data in transit.        \\
	\hline
	NFR-SEC-02      & Role enforcement implemented at middleware level, not inside individual controller methods. & Middleware-level       & Single enforcement point; no gap from missing checks.  \\
	\hline
	NFR-SEC-03      & AuditLog has no UPDATE or DELETE permissions for any role; only INSERT by audit middleware. & INSERT-only            & Records must be tamper-evident and legally defensible. \\
	\hline
\end{longtable}


\subsection{Usability}
Usability requirements recognise that instructors are domain experts in teaching, not in building-

automation software, so the override path in particular must be self-evident.

\begin{longtable}{|L{2.5cm}|L{5.0cm}|L{3.2cm}|L{4.0cm}|}
	\caption{Non-functional requirements --- Usability.} \label{tab:3-19}                                                                                                                \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{NFR-ID} & \textbf{Requirement}                                                                          & \textbf{Design Target}       & \textbf{Rationale}                  \\
	\hline
	...
	\hline
	\endlastfoot
	NFR-USA-01      & A first-time instructor can locate and issue a manual override without reading documentation. & <= 3 clicks                  & Instructors are not IT specialists. \\
	\hline
	NFR-USA-02      & Fault banners visible on LabStatusCard without scrolling, up to 6 cards on screen.            & 1920x1080, no scroll visible & Faults must be immediately visible. \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.5cm}|L{5.0cm}|L{3.2cm}|L{4.0cm}|}
	\caption{Non-functional requirements --- Maintainability.} \label{tab:3-20}                                                                                                                                                                           \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{NFR-ID} & \textbf{Requirement}                                                                                                                              & \textbf{Design Target}   & \textbf{Rationale}                                   \\
	\hline
	...
	\hline
	\endlastfoot
	NFR-MAIN-01     & All control thresholds (LDR zones, YOLO confidence, polling intervals, JWT expiry, fan levels) stored in SystemConfig.                            & 0 hardcoded thresholds   & Enables tuning without code changes or redeployment. \\
	\hline
	NFR-MAIN-02     & Each of the four tiers is independently deployable; updating one tier requires no changes to any other tier, provided the API contract is stable. & Zero cross-tier coupling & Supports iterative improvement of individual tiers.  \\
	\hline
\end{longtable}
...
\begin{longtable}{|L{2.5cm}|L{5.0cm}|L{3.2cm}|L{4.0cm}|}
	\caption{Non-functional requirements --- Scalability.} \label{tab:3-21}                                                                                                                                                                                                        \\
	\hline
	\rowcolor{tableheadergray}
	\textbf{NFR-ID} & \textbf{Requirement}                                                                                                                       & \textbf{Design Target}                                    & \textbf{Rationale}                                  \\
	\hline
	...
	\hline
	\endlastfoot
	NFR-SCAL-01     & System supports >= 20 labs simultaneously active laboratories, each posting sensor data every 5 s, without architecture or schema changes. & Faculty has >10 IT labs; future growth must be supported. & Scalability to full campus estate without redesign. \\
	\hline
	NFR-SCAL-02     & Adding a new laboratory requires only: one Laboratories row, one user account, one ESP32 configured, one camera configured.                & 0 schema changes per new lab                              & Reduces IT Department effort for lab additions.     \\
	\hline
\end{longtable}
...
counterpart of the Entry/Exit Detection (FR-EE), Occupancy Detection and Classification (FR-

OD), Lighting Control (FR-LC), and Ventilation Control (FR-VC) requirement areas, and it

realises use cases UC-09, UC-10, UC-12, and UC-13.


% --- FIGURE PLACEHOLDER: Figure 3.4 ---
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/seq-occupancy-workflow.png}
	\caption{Sequence diagram --- occupancy-triggered automation workflow}
	\label{fig:3-4}
\end{figure}
...
5) and receives it back (step 6); if the camera is not active, this branch is skipped entirely, which is

the sequence-diagram expression of the CAMERA\_UNAVAILABLE fault path defined in FR-

OD-03 --- the diagram shows the happy path, while the fault path is specified formally by that
...
branches that correspond directly to FR-LC-03 and FR-VC-03 on one side and FR-LC-01/FR-VC-

01 on the other: when [students greater than 0], the Controller tells the ESP32 to turn ON devices
...
for YOLO's answer. This is what allows FR-OD-04 to exist at all: a disagreement between the HC-

SR04-based entry/exit signal and the YOLO-based count is only meaningful because both signals
...
implementation side, by the camera-source resolution logic described in the and

modules referenced later in the design chapter.


% --- FIGURE PLACEHOLDER: Figure 3.5 ---
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/seq-camera-streaming.png}
	\caption{Sequence diagram --- live camera streaming workflow}
	\label{fig:3-5}
\end{figure}
...
the create\_capture/\_ensure\_camera\_started logic in the streaming service --- an already-registered
...
rather than a frozen or blank video panel, consistent with the fault-visibility requirements of FR-
...
condition (CAMERA\_UNAVAILABLE / CAMERA\_TIMEOUT) in FR-OD-03 and FR-FH-01
...
% --- FIGURE PLACEHOLDER: Figure 3.6 ---
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/seq-manual-override.png}
	\caption{Sequence diagram --- manual override (relay control) workflow}
	\label{fig:3-6}
\end{figure}
...
and the ASP.NET Backend returns 200 OK with a JWT (step 3) --- the direct execution of FR-AC-

01, which specifies bcrypt password verification and an HMAC-SHA256-signed token expiring in
...
as required by NFR-SEC-02, before any relay logic runs, and it is also the point at which FR-MO-

01's constraint that an instructor may only override a relay in their own assigned lab would be
...
Once the device confirms execution, the backend updates its dashboard data store (step 12, a self-

call corresponding to writing a new RelayStates row with CommandSource = Override, satisfying
...
confirmation that automation has been suspended for that relay, exactly as FR-LC-02 requires; on
	...
	[Failure], the dashboard shows an error message instead (step 15), consistent with the same fail-

visibly principle used in the camera streaming workflow of Section 3.8.2. Independently of which
...
Taken together with the other diagrams, this workflow demonstrates the precedence rule of FR-

MO-02 implicitly: because the override-creation path runs through the same role-validation self-

call at step 7, an administrator's request would be authorised to target any laboratory's relay, while
...
% --- FIGURE PLACEHOLDER: Figure 3.7 ---
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{ch03/figures/seq-transfer-request.png}
	\caption{Sequence diagram --- laboratory transfer request workflow}
	\label{fig:3-7}
\end{figure}
...
authorised, the backend creates a new TransferRequests row with status PENDING (step 8, a self-

call), and returns 201 Created to the Angular Dashboard (step 9), which confirms the submission to
...
Sections 3.6 and 3.7 then fixed the testable requirement baseline for the rest of the project: thirty-

six functional requirements across the nine areas of System Initialisation and Startup, Entry and
...
combine to switch lighting and ventilation automatically. Research on unobtrusive

sensor-based occupancy detection \cite{chen2018} and BLE-integrated spatial proﬁling
\cite{green2024} provides additional validation for the multimodal sensing approach

adopted in GreenLab's design. The live camera streaming workflow
